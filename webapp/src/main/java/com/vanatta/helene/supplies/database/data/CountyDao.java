...
public class CountyDao {
  ...
  public static Map<String, List<String>> fetchFullCountyListing(Jdbi jdbi) {
    ...
    List<Map<String,Object>> results = jdbi.withHandle(handle -> handle.createQuery(query)
        .mapToMap()
        .list());
    ...
    results
        .forEach(entry -> {
          List<String> counties = stateCounties.computeIfAbsent((String) entry.get("state"), _ -> new ArrayList<>());
